...
[FlashAir](https://www.flashair-developers.com/ja/about/overview/) (東芝製の無線 LAN 機能を搭載した SDHC メモリーカード) に保存された写真データを、「ワイヤレスデータ転送」機能を使って無線 LAN 搭載の Macintosh (MacBook Air 等) にダウンロードして日付ディレクトリにアーカイブするシェルスクリプト (bash) です。
...
- -a: アーカイブ動作のみ (-d オプション使用時や、アーカイブをキャンセルして写真データが BASE_DIR 以下の一時保存ディレクトリ上に仮保存されている状態で使用されることを仮定しています)

- -y: 対話的な問い合わせがある場合に全てデフォルトで回答
