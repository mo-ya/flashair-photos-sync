...
5. flashair-photos-sync.sh 内の以下の変数をテキストエディタで編集
    ...
    - アーカイブ後に実施するコマンド (任意)
      ...
      スクリプトによるアーカイブ完了後に任意のコマンドを実行できます。例えば以下のように設定すると、アーカイブ完了後に iPhoto を起動しつつ、`${HOME}/Pictures/FlashAir/` 以下の 2013 や 2014 など 4 桁の数値のフォルダ以下の写真を iPhoto ライブラリに読み込みます。この `POST_SCRIPT` は未設定でも問題ありません。
